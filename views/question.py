...
def get_question(question_id: int, db_session: Session = Depends(get_db)):
...
def get_random_question(db_session: Session = Depends(get_db)):
...
def create_question(question: CreateQuestion,  db_session: Session = Depends(get_db)):
